...
async def project_tasks(
    ...
    number: int,
    status: discord.app_commands.Choice[str] | None = None,
    ...
    project_node_details = (
        None  # To store details like title, fields from the first GQL response
    )
    current_cursor = None
    has_next_page = True
    page_count = 0
    ...
    while has_next_page:
        ...
        if (
            project_node_details is None
        ):  # Store overall project details from the first page
    ...
    sorted_accumulated_items_nodes = sorted(
        accumulated_items_raw_nodes, key=get_created_at, reverse=True
    )

    # --- Determine target status for filtering ---
    effective_display_status: str
    user_selected_status_name: str | None = None
    if status:
        effective_display_status = status.value  # "Todo", "In Progress", etc.
        user_selected_status_name = (
            status.name
        )  # "To Do", "In Progress", etc. (for messages)
    else:
    ...
    if project_fields_nodes:
        for field in project_fields_nodes:
            if field:
                ...
                if (
                    field_typename == "ProjectV2SingleSelectField"
                    and field_name.lower() == STATUS_FIELD_NAME.lower()
                ):
                    ...
                    status_field_options_ordered = [
                        opt["name"]
                        for opt in options
                        if opt and isinstance(opt, dict) and "name" in opt
                    ]
    ...
    if (
        not sorted_accumulated_items_nodes
    ):  # Project is completely empty (no items from API)
        ...
        if not found_status_field:
            ...
            embed.title += (
                f" - {effective_display_status}"  # Add status to title if filtering
            )
            ...
            if (
                effective_display_status in columns_content
            ):  # Check if the status is a valid column
                embed.add_field(
                    name=effective_display_status, value="_(empty)_", inline=False
                )
                ...
                footer_suffix = (
                    f" · Status '{effective_display_status}' not in project options"
                )

    elif found_status_field:
        embed.title = f"{project_base_title} - {effective_display_status}"
        items_in_target_status_full = columns_content.get(
            effective_display_status
        )  # Get all items for this status (already sorted)

        if (
            items_in_target_status_full is not None
        ):  # The target status column exists in the project's setup
            items_to_display = items_in_target_status_full[
                :MAX_ITEMS_TO_DISPLAY
            ]  # Truncate to display up to max newest
            ...
            if not items_to_display:
                ...
                for item_text_original in items_to_display:
                    ...
                    if (
                        len(item_text) > DISCORD_FIELD_CHAR_LIMIT
                    ):  # Truncate individual super long items
                        item_text = item_text[: DISCORD_FIELD_CHAR_LIMIT - 4] + "..."

                    # Length of new item + 1 for newline (if not the first item in chunk)
                    len_of_item_with_newline = len(item_text) + (
                        1 if current_chunk_lines else 0
                    )
                ...
                if not field_chunks_values:
                    embed.add_field(
                        name=field_base_name,
                        value="_(No displayable items)_",
                        inline=False,
                    )
                else:
                    ...
                    for i, chunk_value_str in enumerate(field_chunks_values):
                        ...
                        embed.add_field(
                            name=field_name_display, value=chunk_value_str, inline=False
                        )
        ...
        all_tasks_list_full = columns_content.get(
            "All Tasks", []
        )  # Get all tasks (already sorted)
        items_to_display = all_tasks_list_full[
            :MAX_ITEMS_TO_DISPLAY
        ]  # Truncate to display up to max newest
        ...
        footer_suffix = (
            f" · Showing {num_displayed}/{total_in_category} tasks (All Tasks)"
        )
        ...
        if not items_to_display:
            ...
            if not field_chunks_values:
                embed.add_field(
                    name=field_base_name, value="_(No displayable items)_", inline=False
                )
            else:
                num_chunks = len(field_chunks_values)
                for i, chunk_value_str in enumerate(field_chunks_values):
    ...
    if not embed.fields and not embed.description:
        ...
        embed.description = (
            "No tasks found matching the criteria, or the project is empty."
        )
...
async def tasks(
    ...
    await project_tasks.callback(
        interaction, number=project_number, status=status, _deferred_by_caller=True
    )
