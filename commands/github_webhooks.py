...
async def on_message_webhook(message):
    ...
    if message.channel.id == SOURCE_CHANNEL_ID:
        print(
            f"🔍 Message in source channel from {message.author.name} (ID: {message.author.id})"
        )
        if message.embeds:
            print(f"📋 Message has {len(message.embeds)} embed(s)")
            for i, embed in enumerate(message.embeds):
                print(
                    f"  Embed {i + 1}: {embed.description[:100] if embed.description else 'No description'}..."
                )

    # Only process messages from MyRepoBot in the specific channel
    if (
        message.author.id != MYREPOBOT_ID
...
async def process_issue_notification(message, embed):
    ...
    print(
        f"🔍 Processing {event_type} for issue #{issue_number} in {repo_owner}/{repo_name}"
    )
    ...
    await forward_notification_to_channels(
        message, embed, channels_to_notify, issue_info
    )


def parse_issue_embed(description: str) -> tuple[str, str, int, str] | None:
    """
...
def extract_github_username(description: str) -> str | None:
    """
...
async def forward_notification_to_channels(
    ...
    github_username = (
        extract_github_username(embed.description) if embed.description else None
    )
    ...
    if github_username:
        try:
            ...
            discord_username = member_mapping_cache.get_discord_username(
                github_username
            )
            ...
            if discord_username:
                ...
                discord_user = await bot.reminder_processor.find_discord_user(
                    discord_username
                )
                ...
                if discord_user:
                    ...
                    print(
                        f"👤 Added Discord mention for {github_username} -> {discord_user.mention}{name_display}"
                    )
                else:
                    print(
                        f"❌ Could not find Discord user for {discord_username} (GitHub: {github_username})"
                    )
            else:
    ...
    forwarded_embed.set_footer(
        text=f"Forwarded from project notifications • Issue #{issue_number}"
    )
    ...
    for channel_id in channel_ids:
        try:
            ...
            if channel:
                ...
                mention_info = (
                    " (with mention)"
                    if github_username and enhanced_description != embed.description
                    else ""
                )
                print(
                    f"✅ Forwarded {event_type} notification for issue #{issue_number} to #{channel.name}{mention_info}"
                )
            else:
