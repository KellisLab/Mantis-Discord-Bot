        ...
        if result["success"]:
            ...
            if result["participants"]:
                participant_list = ", ".join(
                    result["participants"][:10]
                )  # Limit to first 10
                if len(result["participants"]) > 10:
                    participant_list += (
                        f" and {len(result['participants']) - 10} others"
                    )
                embed.add_field(
                    name="👥 Participants", value=participant_list, inline=False
                )
            ...
            if result.get("summary"):
                ...
                if len(result["summary"]) > 800:
                    summary_preview += "..."
                embed.add_field(
                    name="📝 Summary Preview", value=summary_preview, inline=False
                )
