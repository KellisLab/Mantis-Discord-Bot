...
async def run_assistant(
    assistant_id: str, user_message: str, timeout_seconds: int = 90
) -> str:
    """
    ...
    try:
        ...
        for repo in repos:
            ...
            if "Mantis" in repo_name:
                issues_url = (
                    f"https://api.github.com/repos/{GITHUB_ORG_NAME}/{repo_name}/issues"
                )
                params = {"state": "open", "assignee": "none"}
                issues_response = requests.get(
                    issues_url, headers=HEADERS, params=params
                )
        ...
        return f"Error retrieving tasks from GitHub: {e!s}"
    if not all_tasks:
...
def assign_task_to_user(github_username: str, issue_url: str) -> str:
    ...
    try:
        ...
        return (
            f"Could not assign you to the task. (GitHub returned status {status_code})"
        )
...
async def recommend_mentors_via_assistant(
    mentors: list, user_interests_text: str, assigned_tasks_text: str
) -> list:
    """
    ...
    mentor_list_text = "\n".join(
        [f"- {m['full_name']} (Teams: {m['teams']})" for m in mentors]
    )
    user_prompt = (
    ...
    if not recommendations:  # Fallback to random mentors
        ...
        return [
            {
                **m,
                "reason": "Recommended as a generally available and experienced mentor.",
            }
...
async def handle_mentor_followup_question(
    user_question: str,
    user_interests: str,
    recommended_mentors: list,
    assigned_task: str = "",
) -> str:
    """
    ...
    mentor_list_text = "\n".join(
        [
            f"- {m['full_name']} (Teams: {m['teams']}) - {m['reason']}"
...
async def draft_outreach_message(
    user_interests_text: str, assigned_tasks_text: str, mentor_name: str
) -> str:
    """
...
class MantisCog(commands.Cog):
    ...
    class M4MView(View):
        ...
        async def find_more_button(
            ...
            if last_msg_id:
                ...
                await last_msg.reply(
                    "Searching for more tasks...", mention_author=False
                )
            else:
            ...
            final_message = (
                "Here are some more tasks you might like:\n\n" + new_tasks
            )[:DISCORD_CHAR_LIMIT]
        ...
        @discord.ui.button(
            label="I have a task, assign me", style=discord.ButtonStyle.success
        )
        async def assign_task_button(
            self, interaction: discord.Interaction, button: Button
        ):
            for item in self.children:
                item.disabled = True
            ...
            try:
                ...
                for gh_username, info in mapping.items():
                    if (
                        ...
                        and info.get("discord_username", "").lower() in lower_candidates
                    ):
    ...
    class MentorButton(Button):
        def __init__(
            ...
            assigned_tasks_text: str,
        ):
            label = (
                mentor_name if mentor_name and mentor_name.strip() else "View Mentor"
            )
        ...
        try:
            if stage == "mentor_interests":
                ...
                sent_message = await message.reply(
                    "Thanks! Let me find mentors who match your interests and skills...",
                ...
                async with message.channel.typing():
                    ...
                    mentors = await loop.run_in_executor(
                        None, get_mentors_from_public_sheet
                    )
                    ...
                    recommended_mentors = await recommend_mentors_via_assistant(
                        mentors, interests, assigned_task_placeholder
                    )

                    # Send context message first
                    skills_explanation = await explain_skills_relation_to_mantis(
                        interests
                    )
                    context_message = (
                        f"## How Your Skills Relate to Mantis\n{skills_explanation}"
                    )
                    ...
                    for mentor in recommended_mentors:
                        ...
                        if (
                            len(mentor_message + mentor_entry) > 1800
                    ...
                    view = self.MentorSelectionView(
                        self,
                        user_id,
                        recommended_mentors,
                        interests,
                        assigned_task_placeholder,
                    )
                    sent_message = await message.channel.send(
                        mentor_message[:DISCORD_CHAR_LIMIT], view=view
                    )
                    session["last_bot_message_id"] = sent_message.id
                ...
                session["user_interests"] = interests
                sent_message = await message.reply(
                    "Thanks! Finding some suitable tasks based on your interests...",
                    mention_author=False,
                )
                session["last_bot_message_id"] = sent_message.id
                ...
                intro = (
                    "Based on what you told me, I think you'll like these tasks:\n\n"
                )

                # Send the final response with buttons in one message
                final_content = (
                    f"{intro}{recommended_tasks}\n\nWhat would you like to do next?"
                )
                button_message = await message.channel.send(
                    final_content[:DISCORD_CHAR_LIMIT], view=self.M4MView(self, user_id)
                )
                ...
                sent_message = await message.reply(
                    "Got it! Now, please reply with the full **GitHub issue URL** you'd like to be assigned to.",
                    mention_author=False,
                )
                session["last_bot_message_id"] = sent_message.id
                self.sessions[user_id] = session
                ...
                if not github_username:
                    sent_message = await message.reply(
                        "I don't have your GitHub username yet. Please send it first.",
                        mention_author=False,
                    )
                    session["last_bot_message_id"] = sent_message.id
                    session["stage"] = "awaiting_github_username"
                ...
                if "has been assigned" in assign_response:
                    ...
                    sent_message = await message.channel.send(
                        "Now that you have a task, let's find you a mentor! Searching..."
                    )
                    session["last_bot_message_id"] = sent_message.id
                    ...
                    async with message.channel.typing():
                        ...
                        view = self.MentorSelectionView(
                            self, user_id, recommended_mentors, interests, tasks
                        )
                        sent_message = await message.channel.send(
                            mentor_message[:DISCORD_CHAR_LIMIT], view=view
                        )
                    ...
                    sent_message = await message.channel.send(
                        "Since the assignment didn't succeed, mentor recommendations are unavailable. You can try assigning another task!"
                    )
                    session["last_bot_message_id"] = sent_message.id
                ...
                if any(
                    ...
                    for word in ["thanks", "thank you", "done", "that's all", "no more"]
                ):
                    await message.reply(
                        "You're welcome! Feel free to reach out to any of the mentors. Good luck with your Mantis contributions!",
                        mention_author=False,
                    )
                ...
                sent_message = await message.reply(
                    "Let me think about that...", mention_author=False
                )
                session["last_bot_message_id"] = sent_message.id

                async with message.channel.typing():
                    response = await handle_mentor_followup_question(
                        user_question, interests, recommended_mentors, assigned_task
                    )
                ...
                sent_message = await message.channel.send(
                    follow_up_message[:DISCORD_CHAR_LIMIT]
                )
                session["last_bot_message_id"] = sent_message.id
                self.sessions[user_id] = session
            ...
            await message.channel.send(
                "Sorry, something went wrong. Please try running the `/m4m` or `/m4m_mentor` command again."
            )
            self.sessions.pop(user_id, None)

    @app_commands.command(
        name="m4m", description="Find a task and mentor to contribute to Mantis."
    )
    ...
    @app_commands.command(
        name="m4m_mentor",
        description="Find a mentor based on your skills and interests.",
    )
    async def m4m_mentor_only(self, interaction: discord.Interaction):
        user_id = interaction.user.id
        self.sessions[user_id] = {
            "stage": "mentor_interests",
