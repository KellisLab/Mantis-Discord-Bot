...
class DmUpdateHandler(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.update_manager = getattr(bot, 'github_update_manager', None)
    ...
    async def on_message(self, message: discord.Message):
        ...
        session = self.update_manager.get_session(message.author.id)
        if not session:
            return
        ...
        try:
            await self.handle_update_conversation(message, self.update_manager, session)
            ...
            self.update_manager.end_session(message.author.id)


    async def handle_update_conversation(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
        """Handle the conversation flow for updating GitHub issues/PRs."""
        stage = session.get("stage", "awaiting_initial_response")
        user_id = message.author.id

        stage_handlers = {
            "awaiting_initial_response": self.handle_initial_response,
            "awaiting_item_selection": self.handle_item_selection,
            "awaiting_item_selection_for_update": self.handle_item_selection_for_update,
            "awaiting_update_content": self.handle_update_content,
            "awaiting_continue_choice": self.handle_continue_choice,
            "awaiting_next_update_or_done": self.handle_next_update_or_done,
        }
    ...
    async def handle_initial_response(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
        ...
        if len(update_items) == 1:
            await self.post_update_to_github(message, update_manager, session, 0, update_content)
            ...
            await self.send_item_selection_for_update(message, update_manager, session, update_content)


    async def handle_item_selection(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
    ...
    async def handle_update_content(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
    ...
    async def handle_continue_choice(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
        ...
        if user_input in ["yes", "y", "continue", "more", "next"]:
            ...
            await self.send_item_selection_prompt(message, update_manager, session)
    ...
    async def send_item_selection_prompt(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
    ...
    async def handle_item_selection_for_update(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
        ...
        try:
            ...
            if 1 <= selection <= len(update_items):
                selected_index = selection - 1
                await self.post_update_to_github(message, update_manager, session, selected_index, pending_update)
                return
    ...
    async def post_update_to_github(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict, item_index: int, update_content: str):
    ...
    async def send_item_selection_for_update(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict, update_content: str):
    ...
    async def handle_next_update_or_done(self, message: discord.Message, update_manager: GitHubUpdateManager, session: Dict):
        ...
        if len(remaining_items) == 1:
            ...
            await self.post_update_to_github(message, update_manager, session, remaining_items[0], update_content)
        else:
...
async def setup(bot):
    """Register the DM update handler Cog with the bot."""
    await bot.add_cog(DmUpdateHandler(bot))
