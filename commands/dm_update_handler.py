...
class DmUpdateHandler(commands.Cog):
    ...
    async def handle_update_conversation(
        ...
        update_manager: GitHubUpdateManager,
        session: dict,
    ):
        ...
        if handler:
            ...
            await message.channel.send(
                "❓ I'm not sure what stage we're at. Let me reset your session."
            )
    ...
    async def handle_initial_response(
        ...
        if not update_items:
            await message.channel.send(
                "❌ No items found in your reminder session. Please wait for the next reminder."
            )
            update_manager.end_session(user_id)
            return
        ...
        if not update_content:
            await message.channel.send(
                "❌ Please provide some content for your update."
            )
            return

        # If only one item, post the update directly
        if len(update_items) == 1:
            await self.post_update_to_github(
                message, update_manager, session, 0, update_content
            )

        else:
            # Multiple items, store the update and ask which item it's for
            update_manager.update_session(
                user_id,
                {
            ...
            await self.send_item_selection_for_update(
                message, update_manager, session, update_content
            )
    ...
    async def handle_update_content(
        ...
        if selected_index is None or selected_index >= len(update_items):
            await message.channel.send(
                "❌ Error: No item selected. Let me reset your session."
            )
            update_manager.end_session(user_id)
            return
        ...
        if success:
            ...
            remaining_items = [
                i for i, item in enumerate(update_items) if i not in updated_items
            ]
    ...
    async def send_item_selection_prompt(
        self,
        message: discord.Message,
        update_manager: GitHubUpdateManager,
        session: dict,
        ...
        remaining_items = [
            item for i, item in enumerate(update_items) if i not in updated_items
        ]
    ...
    async def handle_item_selection_for_update(
        self,
        message: discord.Message,
        update_manager: GitHubUpdateManager,
        session: dict,
        ...
        try:
            ...
            if 1 <= selection <= len(update_items):
                selected_index = selection - 1
                await self.post_update_to_github(
                    message, update_manager, session, selected_index, pending_update
                )
                return
    ...
    async def post_update_to_github(
        self,
        message: discord.Message,
        update_manager: GitHubUpdateManager,
        session: dict,
        ...
        if item_index >= len(update_items):
            await message.channel.send(
                "❌ Error: Invalid item index. Let me reset your session."
            )
            update_manager.end_session(user_id)
            return
    ...
    async def send_item_selection_for_update(
        self,
        message: discord.Message,
        update_manager: GitHubUpdateManager,
        session: dict,
    ...
    async def handle_next_update_or_done(
        self,
        message: discord.Message,
        update_manager: GitHubUpdateManager,
        session: dict,
        ...
        if not update_content:
            await message.channel.send(
                "❌ Please provide some content for your update, or reply with **done** to finish."
            )
            return
        ...
        if not remaining_items:
            await message.channel.send(
                "🎉 You've already updated all your items! Great job!"
            )
            update_manager.end_session(user_id)
            return
        ...
        if len(remaining_items) == 1:
            ...
            await self.post_update_to_github(
                message, update_manager, session, remaining_items[0], update_content
            )
        else:
