...
async def issues(
    ...
    repository: discord.app_commands.Choice[str] | None = None,
):
    ...
    for repo_name in repos_to_query:
        ...
        while has_next_page:
            ...
            if gql_errors:
                error_messages = [
                    err.get("message", "Unknown GraphQL error") for err in gql_errors
                ]
                full_error_msg = (
    ...
    if not issues_to_display:
        embed.add_field(
            ...
            value="No open issues found in the specified repositories.",
            inline=False,
        )
    else:
        ...
        for repo, repo_issues in repo_groups.items():
            ...
            if not field_chunks:
                embed.add_field(
                    name=f"{repo} Issues",
                    value="_(No displayable issues)_",
                    inline=False,
                )
            else:
    ...
    embed.set_footer(
        text=f"Mantis AI Cognitive Cartography · {len(all_issues)} total open issues"
    )
...
async def prs(
    ...
    if not prs_to_display:
        embed.add_field(
            ...
            value=f"No {state_display.lower()} pull requests found in the specified repositories.",
            inline=False,
        )
    else:
        ...
        for repo, repo_prs in repo_groups.items():
            ...
            if not field_chunks:
                embed.add_field(
                    name=f"{repo} Pull Requests",
                    value="_(No displayable pull requests)_",
                    inline=False,
                )
            else:
                num_chunks = len(field_chunks)
    ...
    embed.set_footer(
        text=f"Mantis AI Cognitive Cartography · {len(all_prs)} total {state_display.lower()} PRs"
    )
