...
class TranscriptProcessor:
    ...
    def __init__(
        ...
        self.bot = bot
        self.transcript_api = (
            transcript_api if transcript_api is not None else TranscriptAPI()
        )
        self.message_analyzer = (
            message_analyzer
            if message_analyzer is not None
            else MessageAnalyzer(member_cache)
        )
        self.ai_summarizer = (
            ai_summarizer if ai_summarizer is not None else ConversationSummarizer()
        )

    async def process_channel_transcript(
        self, channel_id: int, hours_back: int = None, force_process: bool = False
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            real_names = await self.message_analyzer.map_users_to_real_names(
                participants
            )

            # Step 7: Format messages for AI analysis
            formatted_conversation = self.message_analyzer.format_messages_for_analysis(
                valid_messages
            )

            # Step 8: Generate AI summary
            raw_summary = await self.ai_summarizer.generate_conversation_summary(
                formatted_conversation, channel.name, real_names
            )
            ...
            formatted_summary = self.ai_summarizer.format_summary_for_api(
                raw_summary, channel.name, len(valid_messages)
            )
            ...
            if success:
                ...
                message = (
                    f"Failed to submit transcript for #{channel.name}: {error_detail}"
                )
                print(f"❌ {message}")
                return {
                    "success": False,
            ...
            print(f"❌ {message}")
            return {"success": False, "message": message, "error": "Permission denied"}
    ...
    async def process_all_configured_channels(self) -> dict[str, tuple[bool, str]]:
        ...
        total = len(results)
        print(
            f"📊 Transcript processing complete: {successful}/{total} channels successful"
        )
        ...
        for channel_id in TRANSCRIPT_CHANNELS:
            try:
                ...
                if not channel.permissions_for(channel.guild.me).read_message_history:
                    errors.append(
                        f"Bot lacks 'Read Message History' permission in #{channel.name}"
                    )

                if not channel.permissions_for(channel.guild.me).view_channel:
                    errors.append(
                        f"Bot lacks 'View Channel' permission in #{channel.name}"
                    )
