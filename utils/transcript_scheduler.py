...
class TranscriptScheduler:
    ...
    def __init__(self, bot: discord.Client, processor: TranscriptProcessor = None):
        ...
        self.bot = bot
        self.processor = (
            processor if processor is not None else TranscriptProcessor(bot)
        )
    ...
    def setup_daily_schedule(self):
        ...
        """
        print(
            f"🕐 Setting up daily transcript schedule for {TRANSCRIPT_SCHEDULE_HOUR:02d}:00 UTC"
        )

        # Configure the task to run daily at the specified hour
        schedule_time = time(
            hour=TRANSCRIPT_SCHEDULE_HOUR, minute=0, second=0, tzinfo=timezone.utc
        )
    ...
    async def run_daily_transcript_job(self) -> dict[str, Any]:
        ...
        print(
            f"🚀 Starting daily transcript job at {job_start_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
        )
        ...
        print(
            f"📋 Processing transcripts for {len(TRANSCRIPT_CHANNELS)} configured channels"
        )
        ...
        for channel_id in TRANSCRIPT_CHANNELS:
            ...
            try:
                ...
                results["channel_results"][str(channel_id)] = {
                    ...
                    "processing_time": (
                        datetime.utcnow() - channel_start_time
                    ).total_seconds(),
                }
                ...
                if result["success"]:
                    ...
                    print(
                        f"✅ Channel {channel_id}: {message_count} messages, {participant_count} participants (ID: {transcript_id})"
                    )
                else:
                ...
                results["channels_processed"] += 1
                results["failed_channels"] += 1
                error_msg = f"Unexpected error processing channel {channel_id}: {e!s}"
                print(f"❌ {error_msg}")
                results["errors"].append(error_msg)
                results["channel_results"][str(channel_id)] = {
                    "success": False,
        ...
        print(
            f"📈 Results: {results['successful_channels']} successful, {results['failed_channels']} failed out of {results['channels_processed']} channels"
        )
    ...
    def get_scheduler_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "task_running": self.daily_transcript_task.is_running()
            if hasattr(self, "daily_transcript_task")
            else False,
            "next_iteration": self.daily_transcript_task.next_iteration
            if hasattr(self, "daily_transcript_task")
            and self.daily_transcript_task.is_running()
            else None,
    ...
    def stop_scheduler(self):
        ...
        if (
            hasattr(self, "daily_transcript_task")
            and self.daily_transcript_task.is_running()
        ):
    ...
    async def test_configuration(self) -> dict[str, Any]:
        ...
        if not TRANSCRIPT_CHANNELS:
            ...
            test_results["errors"].append(
                "No channels configured in TRANSCRIPT_CHANNELS"
            )
            return test_results
            ...
            try:
                ...
                if isinstance(channel, discord.TextChannel):
                    ...
                    test_results["channel_details"][str(channel_id)] = {
                        ...
                        "guild": getattr(channel, "guild", {}).get("name", "unknown")
                ...
                test_results["channels_inaccessible"] += 1
                test_results["errors"].append(
                    f"No permission to access channel {channel_id}"
                )
                test_results["channel_details"][str(channel_id)] = {
        ...
        print(
            f"📋 Configuration test complete: {test_results['channels_accessible']}/{total_channels} channels accessible"
        )
