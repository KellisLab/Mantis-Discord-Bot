...
class GitHubUpdateManager:
    ...
    def create_update_session(
        ...
        try:
            ...
            print(
                f"✅ Created update session for user {discord_user_id} with {len(update_items)} items"
            )
            return True
    ...
    def get_session(self, discord_user_id: int) -> dict | None:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: requests.post(
                    api_url, headers=self.github_headers, json=payload, timeout=30
                ),
            )
            ...
            if response.status_code == 201:
                ...
                return (
                    True,
                    f"✅ Comment posted successfully! [View comment]({comment_url})",
                )

            elif response.status_code == 403:
                return (
                    False,
                    "❌ Permission denied. The bot may not have write access to this repository.",
                )

            elif response.status_code == 404:
                return (
                    False,
                    f"❌ {item_type.title()} #{item_number} not found in {repository} repository.",
                )
    ...
    def format_item_list(
        self, items: list[dict], updated_items: list[int] = None
    ) -> str:
