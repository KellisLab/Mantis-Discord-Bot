        ...
        try:
            ...
            if status_code is not None:
                ...
                if status_code in [429, 502, 503]:
                    ...
                    print(
                        f"⏳ API request failed (status {status_code}), retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries})"
                    )
            ...
            print(
                f"⏳ API request failed, retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries}): {e!s}"
            )
            await asyncio.sleep(delay)
            ...
            print(
                f"⏳ Unexpected error, retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries}): {e!s}"
            )
            await asyncio.sleep(delay)
