...
class ReminderScheduler:
    ...
    async def weekly_reminder_task(self):
        ...
        if current_time.weekday() == 5:  # Saturday
            ...
            print(
                f"⏭️ Skipping reminder job - today is {current_time.strftime('%A')}, not Saturday"
            )
    ...
    async def run_weekly_reminder_job(self) -> dict[str, Any]:
        ...
        print(
            f"🚀 Starting weekly reminder job at {job_start_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
        )
        ...
        if self.processor:
            try:
                ...
                print(
                    f"📬 DMs: {results.get('dm_success', 0)} success, {results.get('dm_failed', 0)} failed"
                )
                print(
                    f"📢 Channel: {results.get('channel_sent', 0)} sent, {results.get('channel_failed', 0)} failed"
                )

            except Exception as e:
                error_msg = f"Error during weekly reminder job: {e!s}"
                print(f"❌ {error_msg}")
        ...
        print(
            f"🎯 Weekly reminder job completed at {job_end_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
        )
    ...
    def get_scheduler_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "task_running": self.weekly_reminder_task.is_running()
            if hasattr(self, "weekly_reminder_task")
            else False,
            "next_iteration": self.weekly_reminder_task.next_iteration
            if hasattr(self, "weekly_reminder_task")
            and self.weekly_reminder_task.is_running()
            else None,
    ...
    def stop_scheduler(self):
        ...
        if (
            hasattr(self, "weekly_reminder_task")
            and self.weekly_reminder_task.is_running()
        ):
