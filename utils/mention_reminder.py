...
class MentionReminder(commands.Cog):
    ...
    def _should_process_message(self, message: discord.Message) -> bool:
        ...
        words = content.split()
        url_count = sum(
            1 for word in words if word.startswith(("http://", "https://", "www."))
        )
    ...
    async def _delete_reminder_after_delay(self, reminder_message: discord.Message):
        ...
        try:
            ...
            print(
                f"🗑️ Auto-deleted mention reminder in #{reminder_message.channel.name}"
            )
            ...
            print(
                f"⚠️ No permission to delete reminder message in #{reminder_message.channel.name}"
            )
    ...
    def _check_rate_limit(self, user_id: int) -> bool:
        ...
        if len(self.recent_reminders) > 500:
            ...
            self.recent_reminders = {
                uid: timestamp
                for uid, timestamp in self.recent_reminders.items()
    ...
    async def _send_mention_reminder(self, message: discord.Message):
        ...
        try:
            ...
            print(
                f"📬 Sent mention reminder to {message.author.name} in #{message.channel.name}"
            )
            ...
            print(
                f"⚠️ No permission to send mention reminder in #{message.channel.name}"
            )
        except discord.HTTPException as e:
            # Other Discord API errors
            print(f"❌ Failed to send mention reminder: {e}")
