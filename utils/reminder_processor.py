...
class ReminderProcessor:
    ...
    def __init__(self, bot: discord.Client, member_cache: MemberMappingCache = None):
        ...
        self.member_cache = (
            ...
            else MemberMappingCache(cache_duration=MEMBER_MAPPING_CACHE_DURATION)
        )
    ...
    async def make_github_api_request(self, query: str, variables: dict):
        ...
        success, result, error = await retry_with_exponential_backoff(
            api_call, max_retries=3, base_delay=1.0
        )
        if success:
            return result
    ...
    async def find_discord_user(self, discord_username: str):
        ...
        def matches_username(user, target_username):
            ...
            if (
                hasattr(user, "global_name")
                and user.global_name
                and user.global_name.lower() == target_lower
            ):
                return True

            # Check display name (for guild members)
            if (
                hasattr(user, "display_name")
                and user.display_name
                and user.display_name.lower() == target_lower
            ):
                return True
    ...
    def determine_issue_reminders(self, issue) -> list[dict]:
        ...
        if assignees:
            for assignee in assignees:
                if assignee and assignee.get("login"):
                    reminded_users.append(
                        {"username": assignee["login"], "reason": "assigned"}
                    )
        else:
            ...
            if author and author.get("login"):
                reminded_users.append(
                    {"username": author["login"], "reason": "created"}
                )
    ...
    def determine_pr_reminders(self, pr) -> list[dict]:
        ...
        if is_draft:
            if author_login:
                reminded_users.append(
                    {"username": author_login, "reason": "draft_creator"}
                )
        elif review_decision == "APPROVED":
            if author_login:
                reminded_users.append(
                    {"username": author_login, "reason": "approved_creator"}
                )
        elif review_decision == "CHANGES_REQUESTED":
            if author_login:
                reminded_users.append(
                    {"username": author_login, "reason": "changes_requested_creator"}
                )
        elif review_decision == "REVIEW_REQUIRED" or not review_decision:
            if reviewer_logins:
                for reviewer_login in reviewer_logins:
                    reminded_users.append(
                        {"username": reviewer_login, "reason": "reviewer"}
                    )
            elif author_login:
                reminded_users.append(
                    {"username": author_login, "reason": "awaiting_review_creator"}
                )
        ...
        try:
            ...
            def draw_wrapped_text(text, font, color, max_width, start_y):
                ...
                for paragraph in text.split("\n"):
                    ...
                    for word in words:
                        ...
                        if (
                            draw.textbbox((0, 0), current_line + word + " ", font=font)[
                                2
                            ]
                            > max_width
                        ):
            ...
            if body:
                ...
                y_offset = draw_wrapped_text(
                    body, body_font, text_color, img_width - 2 * padding, y_offset
                )
            else:
                draw.text(
                    (padding, y_offset),
    ...
    async def send_dm_to_user(self, user, content, files=None):
        ...
        success, result, error = await retry_with_exponential_backoff(
            dm_send, max_retries=3, base_delay=0.5
        )
    ...
    async def send_channel_message(self, channel, content):
        ...
        success, result, error = await retry_with_exponential_backoff(
            channel_send, max_retries=3, base_delay=0.5
        )
        return success, error

    def create_dm_message_content(
        self, github_username: str, discord_username: str, issues: list, prs: list
    ) -> str:
        ...
        message_parts = [
            f"🔔 **Hello {discord_username}{name_display}! You have reminders from GitHub (@{github_username})**"
        ]
            ...
            for pr in prs[:5]:  # Limit to 5 PRs per user
                ...
                if url and number:
                    message_parts.append(
                        f"• {status_emoji} [{repo}#{number}]({url}) {title}"
                    )
                    message_parts.append(f"  *{reason_text}*")
                else:
        ...
        message_parts.append(
            "📋 *Visual summaries with context are attached below for reference.*"
        )
        message_parts.append(
            "*Or you can write your updates manually in the corresponding issues and PRs.*"
        )
        return "\n".join(message_parts)

    async def create_channel_message_content(
        self,
        github_username: str,
        discord_username: str,
        issues: list,
        prs: list,
        should_mention: bool = True,
        ...
        if discord_username and should_mention:
            ...
            header = (
                f"🔔 **{discord_username}**{name_display} (GitHub: @{github_username})"
            )
        else:
    ...
    async def process_reminders(
        ...
        target_discord_user: discord.User | None = None,
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            print(
                f"📊 Cache info: {cache_info['cache_size']} mappings, age: {cache_info['cache_age_seconds']}s"
            )
        ...
        for repo_name in REMINDER_REPOS:
            ...
            while has_next_page:
                ...
                try:
                    data = await self.make_github_api_request(
                        issues_query_template, variables
                    )
                except Exception as e:
                ...
                if page_issues and len(page_issues) > 0:
                    ...
                    if oldest_updated and not self.is_stale(
                        oldest_updated, STALE_ISSUE_DAYS * 2
                    ):
                        break
        ...
        for repo_name in REMINDER_REPOS:
            ...
            while has_next_page:
                ...
                if page_prs and len(page_prs) > 0:
                    ...
                    if oldest_updated and not self.is_stale(
                        oldest_updated, STALE_PR_DAYS * 2
                    ):
                        break
        ...
        for github_username, items in all_user_reminders.items():
            ...
            if target_discord_user:
                ...
                if (
                    not potential_discord_user
                    or potential_discord_user.id != target_discord_user.id
                ):
            ...
            if discord_username:
                ...
                if discord_user:
                    dm_content = self.create_dm_message_content(
                        github_username, discord_username, issues, prs
                    )
                    ...
                    print(
                        f"📋 Creating visual summaries for {len(all_items)} items for {discord_username}..."
                    )
                    for item in all_items[
                        :MAX_REMINDER_SUMMARY_FILES
                    ]:  # Limit summaries to avoid Discord limits
                        ...
                        if summary_image:
                            ...
                            summary_files.append(
                                discord.File(summary_image, filename=filename)
                            )
                            print(f"✅ Visual summary created for {repo}#{number}")
                        else:
                            print(
                                f"⚠️ Failed to create visual summary for {repo}#{number}"
                            )

                    # Send DM with or without visual summaries
                    if summary_files:
                        print(
                            f"📎 Attaching {len(summary_files)} visual summaries to DM"
                        )
                        dm_success, dm_error = await self.send_dm_to_user(
                            discord_user, dm_content, files=summary_files
                        )
                    else:
                        print(
                            "📝 No visual summaries available - sending DM with text only"
                        )
                        # Update message if no summaries available
                        dm_content_no_summaries = dm_content.replace(
                            "📋 *Visual summaries with context are attached below for reference.*\n",
                            "",
                        )
                        dm_success, dm_error = await self.send_dm_to_user(
                            discord_user, dm_content_no_summaries
                        )
                    ...
                    if dm_success:
                        ...
                        update_manager = getattr(
                            self.bot, "github_update_manager", None
                        )
                        if update_manager:
                            ...
                            if session_created:
                                print(
                                    f"✅ Created GitHub update session for {discord_username} ({github_username})"
                                )
                            else:
                                print(
                                    f"⚠️ Failed to create GitHub update session for {discord_username}"
                                )
                    else:
            ...
            channel_success, channel_error = await self.send_channel_message(
                fallback_channel, channel_content
            )
        ...
        return {"users_processed": total_users, "delivery_stats": delivery_stats}
