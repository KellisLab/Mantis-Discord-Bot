...
class TranscriptAPI:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            print(
                "⚠️ Warning: M4M_DISCORD_API_KEY not set. Transcript API calls will fail."
            )
    ...
    async def create_discord_transcript(
        ...
        description: str,
        timestamp: datetime,
        people_involved_names: list[str] | None = None,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, headers=headers) as response:
                    ...
                    if response.status == 201:
                        print(
                            f"✅ Created transcript for #{channel_name} (ID: {response_data.get('data', {}).get('id', 'unknown')})"
                        )
                        ...
                        print(
                            f"❌ Failed to create transcript for #{channel_name}: {error_msg}"
                        )
        ...
        try:
            ...
            print(f"❌ Unexpected error in transcript API call: {e!s}")
