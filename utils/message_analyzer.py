...
class MessageAnalyzer:
    ...
    def __init__(self, member_cache: MemberMappingCache = None):
        ...
        """
        self.member_cache = (
            member_cache if member_cache is not None else MemberMappingCache()
        )

    async def fetch_channel_messages(
        self, channel: discord.TextChannel, hours_back: int = None
        ...
        print(
            f"📥 Fetching messages from #{channel.name} since {cutoff_time.strftime('%Y-%m-%d %H:%M:%S')} UTC"
        )
    ...
    def extract_participants(
        ...
        print(
            f"👥 Found {len(participant_list)} participants: {[p.display_name for p in participant_list]}"
        )

        return participant_list

    async def map_users_to_real_names(
        self, discord_users: list[discord.Member]
    ) -> list[str]:
        """
            ...
            for username in possible_usernames:
                if username in discord_to_real_name:
                    ...
                    if real_name not in real_names:  # Avoid duplicates
                        ...
                        print(
                            f"✅ Mapped {user.display_name} ({username}) → {real_name}"
                        )
                        mapped = True
                        break
        ...
        if unmapped_users:
            print(
                f"⚠️ Could not map {len(unmapped_users)} users to real names: {unmapped_users}"
            )
    ...
    def filter_valid_messages(
        ...
        print(
            f"🔍 Filtered {len(messages)} messages → {len(valid_messages)} valid messages"
        )
        return valid_messages

    def check_minimum_threshold(
        self, messages: list[discord.Message], min_count: int = None
    ) -> bool:
        """
        ...
        if meets_threshold:
            print(
                f"✅ Message count ({len(messages)}) meets minimum threshold ({min_count})"
            )
        else:
            print(
                f"❌ Message count ({len(messages)}) below minimum threshold ({min_count}) - skipping transcript"
            )
    ...
    def format_messages_for_analysis(self, messages: list[discord.Message]) -> str:
        ...
        print(
            f"📝 Formatted {len(messages)} messages for AI analysis ({len(conversation_text)} characters)"
        )
