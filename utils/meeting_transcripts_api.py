...
class MeetingTranscriptsAPI:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            print(
                "⚠️ Warning: M4M_DISCORD_API_KEY not set. Meeting transcripts API calls will fail."
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    ...
                    if response.status == 200:
                        print(
                            f"✅ Fetched {len(response_data.get('data', []))} meeting transcripts"
                        )
    ...
    def smart_truncate_for_openai(
        self, formatted_data: dict[str, Any], max_size_mb: float = 0.8
    ) -> dict[str, Any]:
        """
        ...
        print(
            f"⚠️ Response too large ({current_size / 1024 / 1024:.1f}MB), truncating to {max_size_mb}MB"
        )
        ...
        while left <= right:
            ...
            test_data = {**formatted_data, "transcripts": sorted_transcripts[:mid]}
        ...
        truncated_data["meetings_summary"]["truncation_note"] = (
            f"Response truncated to {best_count} most recent transcripts to fit size limits"
        )

        final_size = len(json.dumps(truncated_data).encode("utf-8"))
        print(
            f"✅ Truncated to {best_count} transcripts ({final_size / 1024 / 1024:.1f}MB)"
        )
    ...
    def format_transcripts_for_assistant(
        ...
        for transcript in transcripts_data:
            ...
            formatted_transcript = {
                ...
                "meeting": {
                    ...
                    "date": meeting.get("meeting_date")[:10]
                    if meeting.get("meeting_date")
                    else None,  # Date only
